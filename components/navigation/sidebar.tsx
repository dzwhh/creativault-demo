...
const sections: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      { id: 'market-analysis', label: 'Market Analysis', href: '/agent/market', icon: LineChartIcon },
      { id: 'competitive-analysis', label: 'Competitive Analysis', href: '/agent/competition', icon: ShieldHalfIcon },
      { id: 'creative-analysis', label: 'Creative Analysis', href: '/agent/creative', icon: SparklesIcon },
    ],
  },
  {
    label: 'Tools',
  ...
  },
  {
    label: 'Community',
    items: [
      { id: 'academy', label: 'Academy', href: '/community/academy', icon: AcademyIcon },
      { id: 'blog', label: 'Blog', href: '/community/blog', icon: BlogIcon },
    ],
  },
  {
    label: 'My Page',
...
export function Sidebar() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('menuVisibilityChanged', handleMenuVisibilityChange);
    return () => {
      window.removeEventListener('menuVisibilityChanged', handleMenuVisibilityChange);
    };
  }, []);
      ...
      <nav className="flex-1 overflow-y-auto px-2 pb-4 space-y-6">
        {sections.map((section, si) => {
          ...
          const visibleItems = section.items.filter(item => 
            !item.id || visibleMenuItems.has(item.id)
          );
          
          // 如果这个分组没有可见的菜单项，则不显示整个分组
